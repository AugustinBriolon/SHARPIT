...
vi.mock('@/lib/ai', () => ({ isCoachConfigured: vi.fn().mockReturnValue(false) }));
vi.mock('@/lib/coach/plan/coach-analysis', () => ({ analyzePlannedSession: vi.fn() }));
vi.mock('@/lib/analysis/analysis-run-store', () => ({ withAnalysisRun: vi.fn() }));

import { autoLinkActivitiesOfDay } from '@/lib/planned-session/linking/session-linking';
