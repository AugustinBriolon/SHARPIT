...
function assertStoredDemoReading() {
...
function assertDemoSessionReading(queryClient: QueryClient) {
...
function assertDemoActivityReading(queryClient: QueryClient) {
...
function assertDemoSessionLinkReading(queryClient: QueryClient) {
  assertStoredDemoReading();
  assertDemoSessionReading(queryClient);
  assertDemoActivityReading(queryClient);
}
...
describe('demo-session-link-reading', () => {
  ...
  it('stores canned analysis + narrative without calling the API', () => {
    const queryClient = seedDemoSessionStorage();
    applyDemoSessionLinkReading(queryClient, 'ps-1', 'act-1');
    assertDemoSessionLinkReading(queryClient);
  });
});
