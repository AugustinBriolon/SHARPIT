...
import { tonightReason } from '@sharpit/app/lib/sleep/tonight-reason';
import { cn } from '@sharpit/app/lib/utils';

const RELAXATION_LEAD_MIN = 30;

function TonightPlanDetails({ bedtime, durationMin }: { bedtime: number; durationMin: number }) {
  const relaxation = bedtime - RELAXATION_LEAD_MIN;
  const wake = bedtime + durationMin;
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="text-muted-foreground mt-2 text-sm leading-relaxed">
          Vise {formatDuration(durationMin)} · réveil ~{formatClock(wake % 1440)} · détente dès{' '}
          {formatClock(relaxation)}.
        </p>
      </div>
    </div>
  );
...
function TonightPlanBody({
  ...
  return (
    <>
      {hasPlan && bedtime !== null ? (
        <TonightPlanDetails bedtime={bedtime} durationMin={durationMin} />
      ) : (
