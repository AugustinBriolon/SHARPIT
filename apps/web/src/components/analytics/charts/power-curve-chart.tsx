'use client';

import { Zap } from 'lucide-react';
import { CartesianGrid, Line, LineChart, Tooltip, XAxis, YAxis } from 'recharts';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { InkEmptyState } from '@/components/ui/ink-empty-state';
import { ResponsiveChartFrame } from '@/components/ui/charts/responsive-chart-frame';
import type { PowerCurvePoint } from '@sharpit/app/lib/training/records/record-types';
import { CHART_RECORD_STROKE, CHART_TICK_COLOR } from '@sharpit/app/lib/theme/chart-theme';

interface PowerCurveChartProps {
  data: PowerCurvePoint[];
}

function ChartTooltip({
  active,
  payload,
}: {
  active?: boolean;
  payload?: Array<{ payload: PowerCurvePoint }>;
}) {
  if (!active || !payload?.length) {
    return null;
  }
  const point = payload[0].payload;
  return (
    <div className="analysis-panel rounded-analysis px-3 py-2 text-xs shadow-none">
      <p className="text-muted-foreground mb-1 font-medium">{point.label}</p>
      <p className="text-foreground font-mono text-sm font-semibold">{point.watts} W</p>
      {point.title && <p className="text-muted-foreground mt-1 max-w-44 truncate">{point.title}</p>}
    </div>
  );
}

export function PowerCurveChart({ data }: PowerCurveChartProps) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base font-medium">Courbe de puissance</CardTitle>
        <p className="text-muted-foreground text-xs">
          Puissance maximale moyenne soutenue par durée (vélo)
        </p>
      </CardHeader>
      <CardContent>
        {data.length === 0 ? (
          <InkEmptyState
            description="Ouvre des sorties vélo avec capteur de puissance pour alimenter la courbe."
            icon={Zap}
            title="Aucune donnée de puissance"
            compact
          />
        ) : (
          <ResponsiveChartFrame height={288}>
            <LineChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid stroke="var(--analysis-grid)" strokeDasharray="3 3" />
              <XAxis
                axisLine={false}
                dataKey="label"
                tick={{ fill: CHART_TICK_COLOR, fontSize: 11 }}
                tickLine={false}
              />
              <YAxis
                axisLine={false}
                tick={{ fill: CHART_TICK_COLOR, fontSize: 11 }}
                tickLine={false}
                unit=" W"
                width={44}
              />
              <Tooltip content={<ChartTooltip />} />
              <Line
                dataKey="watts"
                dot={{ r: 3, fill: CHART_RECORD_STROKE }}
                name="Puissance"
                stroke={CHART_RECORD_STROKE}
                strokeWidth={2}
                type="monotone"
              />
            </LineChart>
          </ResponsiveChartFrame>
        )}
      </CardContent>
    </Card>
  );
}
