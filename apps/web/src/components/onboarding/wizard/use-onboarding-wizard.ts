...
import { useOnboardingWizardEffects } from '@/components/onboarding/wizard/use-onboarding-wizard-effects';
import {
  useOnboardingWizardHandlers,
  useOnboardingWizardNavigation,
} from '@/components/onboarding/wizard/use-onboarding-wizard-handlers';
import {
  parseInitialOnboardingStep,
  useOnboardingWizardState,
  type OnboardingWizardInitialProps,
} from '@/components/onboarding/wizard/use-onboarding-wizard-state';
import { useGoalMutations } from '@/hooks/use-data';

export type CredentialProvider = Extract<
  import('@sharpit/app/lib/integrations/shared/client-sync').IntegrationId,
