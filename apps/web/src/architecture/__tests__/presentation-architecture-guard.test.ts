...
const FORBIDDEN_ALIASES = {
  inference: '@sharpit/core/inference',
...
const FORBIDDEN_RESOLVED_PREFIXES: Array<{ prefix: string; reason: string }> = [
  { prefix: path.join(CORE_SRC, 'inference'), reason: 'inference' },
...
function isForbiddenByResolvedPath(resolvedAbsPath: string): string | null {
  const allowedProductInsightTypesResolved = path.join(CORE_SRC, 'product-insight', 'types.ts');
