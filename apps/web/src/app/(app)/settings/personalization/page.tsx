import { Suspense } from 'react';
import { MobileBackLink } from '@/components/layout/header/mobile-back-link';
import { StickyHeader } from '@/components/layout/header/sticky-header';
import { AppearanceDisplayModePicker } from '@/components/settings/appearance';
import { DataHistoryWindowPicker } from '@/components/settings/personalization/data-history-window-picker';
import { SurfaceModulesPanel } from '@/components/settings/personalization/surface-modules-panel';
import { SettingsDemoBlock } from '@/components/settings/settings-demo-block';
import { Skeleton } from '@/components/ui/skeleton';
import { isDemoSession } from '@/lib/demo/demo-session';
import { MOI_HUB_PATH } from '@/lib/moi/paths';

function ExpertModeSectionSkeleton() {
  return (
    <div className="space-y-3" aria-busy>
      <Skeleton className="h-[4.5rem] w-full rounded-xl" />
      <Skeleton className="h-[4.5rem] w-full rounded-xl" />
    </div>
  );
}

function ExpertModeExplainer() {
  return (
    <section className="analysis-panel rounded-analysis-lg space-y-3 px-4 py-4">
      <div>
        <p className="text-label">Ce que ça change</p>
        <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
          Rien n&apos;est calculé différemment — le Digital Twin tourne exactement pareil, que tu
          sois en lecture essentielle ou experte. Seul l&apos;affichage change : les métriques
          techniques restent en retrait tant que tu ne les as pas demandées.
        </p>
      </div>
      <div>
        <p className="text-label">Ce que le mode Expert révèle, page par page</p>
        <ul className="text-muted-foreground mt-1 list-disc space-y-1.5 pl-4 text-sm leading-relaxed">
          <li>
            <span className="text-foreground font-medium">Activités</span> — TSS, IF, puissance
            normalisée, découplage cardiaque et les autres métriques dérivées du stream.
          </li>
          <li>
            <span className="text-foreground font-medium">Charge</span> — courbe PMC (CTL/ATL/TSB)
            et le détail de la charge chronique/aiguë.
          </li>
          <li>
            <span className="text-foreground font-medium">Progression</span> — courbe de puissance
            et les autres courbes de référence.
          </li>
          <li>
            <span className="text-foreground font-medium">Calibration</span> — tes seuils bruts
            (FTP, allure seuil, FC seuil…) et leur historique restent éditables ; le Mode Expert
            change surtout la densité des lectures dérivées.
          </li>
        </ul>
      </div>
      <div>
        <p className="text-label">Sur quelles pages tu verras l&apos;indicateur</p>
        <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
          Un badge « Mode Expert » apparaît en haut des pages techniques dès que la bascule est
          activée. Il ramène ici en un tap.
        </p>
      </div>
    </section>
  );
}

async function ExpertModeSection() {
  if (await isDemoSession()) {
    return (
      <SettingsDemoBlock description="La densité de lecture est une préférence de compte réel. Désactivée sur le compte démo partagé." />
    );
  }

  return (
    <section className="space-y-3" id="densite">
      <div>
        <h2 className="text-section-title">Densité de lecture</h2>
        <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
          Une seule bascule pour révéler ou masquer la couche technique.
        </p>
      </div>
      <AppearanceDisplayModePicker />
      <ExpertModeExplainer />
    </section>
  );
}

export default function SettingsPersonalizationPage() {
  return (
    <div className="space-y-6">
      <MobileBackLink fallbackHref={MOI_HUB_PATH} fallbackLabel="Réglages" showOnDesktop />
      <StickyHeader>
        <p className="text-label">Réglages</p>
        <h1 className="text-page-title mt-1">Personnalisation</h1>
        <p className="text-muted-foreground mt-1 text-sm">
          Densité de lecture, profondeur d&apos;historique et modules visibles dans la navigation.
        </p>
      </StickyHeader>

      <Suspense fallback={<ExpertModeSectionSkeleton />}>
        <ExpertModeSection />
      </Suspense>

      <section className="space-y-3">
        <div>
          <h2 className="text-section-title">Fenêtre de données</h2>
          <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
            Jusqu&apos;où remonter dans les historiques (1 à 5 ans). Appliqué progressivement aux
            surfaces de lecture.
          </p>
        </div>
        <DataHistoryWindowPicker />
      </section>

      <section className="space-y-3">
        <div>
          <h2 className="text-section-title">Modules</h2>
          <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
            Choisis ce que tu veux voir dans la navigation. Journal ouvre `/journal` (contexte du
            jour).
          </p>
        </div>
        <SurfaceModulesPanel />
      </section>
    </div>
  );
}
