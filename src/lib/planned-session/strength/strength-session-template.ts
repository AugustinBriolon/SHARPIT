...
export function planStrengthSessionShape(
  ...
  const minutes = (durationMin !== undefined && durationMin !== null) && durationMin > 0 ? durationMin : DEFAULT_SESSION_MINUTES;
...
function setSeconds(set: EstimatableSet): number {
  ...
  const restSec =
    set.restMode === 'time' && (set.restSec !== undefined && set.restSec !== null) && set.restSec > 0
      ? set.restSec
