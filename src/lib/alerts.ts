...
export function computeAlerts(params: {
  ...
  health: AlertHealth[];
  physicalNotes: { title: string; severity: number | null; status: string }[];
  refDate?: Date;
}): SmartAlert[] {
  ...
  const health = sortedByDateDesc(
    (params.health ?? []).filter((h) => new Date(h.date) <= refDate),
  );
  ...
  if (
    latest?.hrvStatus &&
    /UNBALANCED_LOW|LOW|POOR/i.test(latest.hrvStatus)
  ) {
    alerts.push({
      id: "hrv-unbalanced",
      severity: "warning",
  ...
  const severePain = physicalNotes
    .filter((n) => n.status !== "RESOLVED" && (n.severity ?? 0) >= 6)
    .sort((a, b) => (b.severity ?? 0) - (a.severity ?? 0))[0];
  if (severePain) {
    alerts.push({
      id: "pain-severe",
      severity: "danger",
      title: `Douleur élevée : ${severePain.title}`,
