 ...
 */
export async function buildCoachContext(refDate: Date = new Date()) {
...
export function formatCoachContext(ctx: CoachContext): string {
  ...
  const healthBits = [
    ...
    h.avgSleepMin != null ? `sommeil moy 7j ${Math.floor(h.avgSleepMin / 60)}h${(h.avgSleepMin % 60).toString().padStart(2, "0")}` : null,
  ...
  if (ctx.primaryRace) {
    lines.push(
      `Course principale : ${ctx.primaryRace.title}${ctx.primaryRace.location ? ` (${ctx.primaryRace.location})` : ""} dans ${ctx.primaryRace.daysToGo} jours (~${Math.round(ctx.primaryRace.daysToGo / 7)} semaines).`,
    );
  } else {
    lines.push("Pas de course planifiée.");
  }
  for (const r of ctx.races.slice(1)) {
    lines.push(`Autre course : ${r.title} dans ${r.daysToGo} j.`);
  }
  ...
  if (ctx.recent.length) {
    ...
    for (const a of ctx.recent) {
      const extra = [
        ...
        .join(" · ");
      lines.push(
        `- ${a.date} · ${a.type} ${a.title} (${a.duration})${extra ? ` — ${extra}` : ""}`,
      );
    }
  }
  ...
  if (ctx.physical.length) {
    lines.push(
      "\n## Condition physique à respecter (douleurs / blessures / mobilité)",
    );
    lines.push(
      "IMPÉRATIF : adapte les séances pour ne pas aggraver ces points. Évite de charger les zones douloureuses, propose du renfo/mobilité ciblé si pertinent, réduis l'intensité si nécessaire.",
    );
