...
function evaluateSafetySignal(input: OutcomeEvaluationInput): OutcomeEvaluation['safetySignal'] {
  if (input.conditionObservations.length === 0) {
    return null;
  }
...
function buildEvaluationLimitations(input: OutcomeEvaluationInput): string[] {
  ...
  const daysWithData = countRecoveryDaysWithData(input);
...
function countEvidenceCategories(evaluation: {
  executionMatch: OutcomeEvaluation['executionMatch'];
  subjectiveResponse: OutcomeEvaluation['subjectiveResponse'];
  shortTermRecoveryResponse: OutcomeEvaluation['shortTermRecoveryResponse'];
  safetySignal: OutcomeEvaluation['safetySignal'];
}): number {
  return [
    evaluation.executionMatch,
...
export function evaluateOutcome(input: OutcomeEvaluationInput): OutcomeEvaluation {
  const limitations = buildEvaluationLimitations(input);
  const executionMatch = evaluateExecutionMatch(input);
  const subjectiveResponse = evaluateSubjectiveResponse(input);
  const shortTermRecoveryResponse = evaluateShortTermRecoveryResponse(input);
  const safetySignal = evaluateSafetySignal(input);
  ...
  const evidenceCount = countEvidenceCategories({
    executionMatch,
    subjectiveResponse,
