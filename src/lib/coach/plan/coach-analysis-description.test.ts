...
} from '@/lib/coach/plan/coach-analysis';
...
vi.mock('@/lib/integrations/strava/strava', () => ({
...
describe('resolveAthleteDescription', () => {
  it('skips Strava when local notes are substantial', async () => {
    const { fetchActivityDetail } = await import('@/lib/integrations/strava/strava');
  ...
  it('fetches Strava when local notes are short', async () => {
    const { fetchActivityDetail } = await import('@/lib/integrations/strava/strava');
    const { getValidAccessToken } = await import('@/lib/integrations/strava/strava-sync');
