...
import type { EndurancePrescription } from '@/lib/planned-session/endurance-prescription';
...
describe('garmin-workout-push-state', () => {
  ...
  it('clears receipt when prescription or date changes', () => {
    ...
    expect(
      garminPushClearOnSessionChange({ endurancePrescription: { version: 1, blocks: [] } }),
    ).toEqual(cleared);
    expect(garminPushClearOnSessionChange({ date: new Date() })).toEqual(cleared);
  });
});
