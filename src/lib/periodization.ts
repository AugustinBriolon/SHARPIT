...
export function generateMacroPlan(params: {
  ...
  const startDate = startOfWeek(
    startOfDay(params.startDate ?? new Date()),
    WEEK_OPTS,
  );
  const raceWeek = startOfWeek(startOfDay(params.raceDate), WEEK_OPTS);
  const totalWeeks =
    differenceInCalendarWeeks(raceWeek, startDate, WEEK_OPTS) + 1;
  ...
  for (const block of phaseBlocks) {
    for (let i = 0; i < block.weeks; i++) {
      ...
      targetLoad = Math.max(
        block.phase === "RACE" ? 20 : 80,
        Math.min(900, targetLoad),
      );
...
export function findPlanWeekForDate<
  T extends { weekStart: Date },
>(weeks: T[], date: Date): T | undefined {
