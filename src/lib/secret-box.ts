...
export class SecretDecryptError extends Error {
  constructor(
    message = 'Stored secret is missing, malformed, or encrypted with a different key',
...
export function decryptSecret(encoded: string): string {
  if (!isEncryptedSecret(encoded)) {
    throw new SecretDecryptError('Stored secret is empty or too short to be AES-GCM ciphertext');
  }
  ...
  try {
    ...
    throw new SecretDecryptError(
      'Stored secret could not be decrypted (wrong key or corrupted ciphertext)',
      { cause: error },
    );
  }
}
