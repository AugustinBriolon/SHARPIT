...
function MacroPlanCreateForm({
  ...
  const selectedGoal = datedGoals.find((g) => g.id === goalId);
  const selectLabel =
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Select value={goalId} onValueChange={(v) => onGoalChange(v ?? NO_GOAL)}>
          <SelectTrigger>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button disabled={isGenerating || goalId === NO_GOAL} onClick={onGenerate}>
          {isGenerating ? (
            <>
