...
export type TrendWindowId = (typeof TREND_WINDOWS)[number]['id'];
...
export function CompositionDetailList({
  cards,
  onExplain,
}: {
  cards: BodyMetricCardVm[];
...
export function windowDeltaDisplay(
...
export function CompositionTrendChart({
  ...
  onTrendWindowChange: (id: TrendWindowId) => void;
}) {
  return (
    <>
          ...
          {TREND_WINDOWS.map((w) => {
            ...
            return (
              <button
                ...
                onClick={() => onTrendWindowChange(w.id)}
              >
                {w.label}
              </button>
            );
...
export function filterChartData(
  chartData: BodyChartPoint[] | undefined,
  days: number | null,
): BodyChartPoint[] {
  return chartData?.length ? filterCompositionSeriesByDays(chartData, days) : [];
}
