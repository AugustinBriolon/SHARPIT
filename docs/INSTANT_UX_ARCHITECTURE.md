...
| --------------------------------- | --------------------------------------------- | ------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `useActivities`                   | GET `/api/activities`                         | stale 2m            | Prefetch `/training`,`/coach` · SWR · BG on focus | **Keep.** Prefer patch on CRUD over full invalidate when possible. Training hub (`/training`) keeps chrome; value micro-skeletons on cold load only (`isPending`). |
...
| `useHealthEntries`                | GET `/api/health`                             | stale 2m + previous | SWR · BG after provider sync                      | **Keep.** Prefetch on `/biology` / recovery entry.                                                                                                                 |
| `useBodyComposition`              | GET `/api/body-composition`                   | stale 2m + previous | SWR                                               | **Keep.** Prefetch `/biology`.                                                                                                                                     |
...
| `useRecoveryViewModel`           | `/presentation/recovery`              | `/biology`,`/recovery`                    | **Keep.** Chrome stable; value micro-skeletons on `isPending` / `isPlaceholderData` (date change).                     |
...
3. Prefetch every primary nav target via `usePrefetchNavQuery` (canonical hrefs: `/`, `/training`, `/biology`, `/coach`, `/settings` — legacy aliases still accepted).
...
6. **One cold skeleton per surface — owned by the page, never by a route "safety" boundary.** Do not add hub `loading.tsx` that duplicates page-owned micro-skeletons; Instant UX hubs (Today, Coach, Biology, Training, Planning) and physio drill-downs have **no** route `loading.tsx`. The fix is not a second route-level skeleton system — the Suspense **fallback must match the same chrome + value micro-skeleton contract** the page uses on cold mount (`*LoadingShell` / `loading` / query pending). Keep `Suspense` only when required (e.g. `useSearchParams`). An **empty** fallback is allowed **only** when (a) the suspending child renders synchronously on client navigation **and** (b) all athlete-visible chrome for that route already lives outside the boundary (or in a non-empty fallback). Settings / activity-detail may keep a single route skeleton until those screens get the same page-owned micro-skeleton contract.
