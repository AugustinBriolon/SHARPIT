...
- **Runs** on PRs and `main` pushes whenever non-docs paths change (includes `src/`, lockfiles, `.env*`, workflows, config).
- **Skipped** on pure docs / screenshot / markdown-only changes.
- Requires repository secret `GITGUARDIAN_API_KEY` (workflow fails closed if missing when the job runs).
