...
The proxy 403 is the real security boundary regardless of UI, but `/settings/**` (identity, integrations, equipment) is both meaningless in a demo and the one place a visitor could otherwise be invited to authenticate _their own_ real provider account against the shared demo tenant. `src/app/(app)/settings/layout.tsx` renders a short "Indisponible en démo" state for the whole settings section when `isDemoSession()` is true, wrapped in `<Suspense>` — awaiting `cookies()`/`auth()` outside Suspense blocks the route from prerendering (Next 16's `blocking-prerender-runtime` diagnostic). The Coach page (`src/app/(app)/coach/page.tsx`) gets the same treatment for its send-message flow: the API is already POST-blocked, but an empty conversation with a composer that silently 403s is a bad dead end, so demo visitors see a short explanation instead.
...
- **Credibility.** The seed gained real Paris-area GPS routes (`ActivityStream` rows matching the exact write shape in `src/lib/streams/streams.ts`, `[lat, lng]` ordering), hand-written narrative text (`Activity.narrativeAnalysis`, no AI call — the field is stored, not generated at render time), a `BodyCompositionMeasurement` trend, and one non-alarming `Condition`/`FunctionalCapacity` row so Corps & Santé isn't empty. One seeded bike session deliberately has **no** `ActivityStream` (a home-trainer ride) — this is the same, confirmed-correct "no GPS on indoor rides" behavior real Zwift/Garmin data has (traced end to end in `src/components/training/activity/insights/activity-insights.tsx` while investigating this feedback) — not a bug, so the demo shows it rather than hides it. Journal + Analyses are seeded too (`seedDemoJournalAnalyses`): 14 days of `AthleteDayJournal` with a daily supplement stack, intermittent `late_meal` / `device_in_bed`, modest caffeine, and DailyHealth sleep/recovery/BB aligned so `/journal/analyses` and the Today habit bridge show a credible priority finding. Demo tenant is `PRO` with matching `journalPrefs`.
...
- `src/lib/dev/dev-auth.ts` — the `isDevClerkBypass()` precedent this ADR's bypass pattern follows.
