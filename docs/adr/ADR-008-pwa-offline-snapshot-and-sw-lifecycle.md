...
- Reconnect never triggers a hard `location.reload()` — that would wipe the in-memory React Query cache and force cold skeletons on every flaky mobile reconnect, contradicting Instant UX. Reconnect refresh stays with TanStack Query's `refetchOnReconnect` instead. This was originally expressed as Serwist's `reloadOnOnline: false`; since [ADR-009](./ADR-009-turbopack-build-and-serwist-configurator.md) moved Serwist to configurator mode there is no injected client entry to reload at all, so the guarantee is now structural rather than a setting.
