...
| Coach conversationnel             | `/coach`                              | `components/coach/` (+ kit `components/coach/kit/`)                                       | `lib/coach/`, `lib/coach-memory/`, `lib/decision-memory/`                                                        | Kit = primitives chat ; re-exports temporaires `components/agents/`                          |
...
| Kit chat coach (reachable only)                  | `components/coach/kit/`                           | Re-exports `components/agents/` ; ne pas réintroduire sidebar / image-gen / file-diff |
...
6. **Guards** : `lib` n’importe pas `@/components` ni `@/core/inference` hors `engines/` sauf allowlist grandfather (`lib-boundary-guard.test.ts`).
