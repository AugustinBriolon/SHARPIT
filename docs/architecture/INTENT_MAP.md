...
7. **`fetch` dans `components/`** : interdit — passer par `hooks` → `lib/query/fetchers/` (ou `sendJson` / `fetchJson`). **P5** : migrations JSON + wrappers SSO/privacy/keepalive sous `fetchers/` ; **exception restante documentée** = custom `fetch` streaming coach (`components/coach/chat/shell/use-coach-chat.ts`, transport AI SDK + budget header). `training/periodization/week-strip` = module + test seuls (candidat purge ultérieure).
