...
import { buildScenarioComparisonPresentationViewModel } from '@sharpit/server/lib/presentation/scenario/scenario-comparison';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('[api/presentation/scenario-comparison]', error);
    return NextResponse.json(
      { error: 'Impossible de produire la comparaison de scénarios' },
      { status: 500 },
    );
  }
}
