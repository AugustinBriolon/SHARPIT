...
vi.mock('@sharpit/core/training/training-day', async (importOriginal) => ({
...
vi.mock('@sharpit/server/lib/journal/journal-habit-experiment-load', async (importOriginal) => ({
  ...(await importOriginal<
    typeof import('@sharpit/server/lib/journal/journal-habit-experiment-load')
  >()),
...
async function givenExperiments(list: EvaluatedExperiment[]) {
  const { loadJournalHabitExperiments } =
    await import('@sharpit/server/lib/journal/journal-habit-experiment-load');
...
describe('/api/journal/habit-experiments', () => {
  ...
  it('stops only a running test of the signed-in athlete', async () => {
    ...
    const { prisma } = await import('@sharpit/db/client');
    const { PATCH } = await import('./[id]/handler');
