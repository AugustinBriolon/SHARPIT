import { formatDate, formatDayRange, formatDistance } from '@sharpit/server/lib/format';
...
function sumNullable(values: Array<number | null | undefined>): number | null {
  const nums = values.filter((v): v is number => isSet(v) && Number.isFinite(v));
  if (nums.length === 0) {
    return null;
  }
...
function computeTripEndAt(ordered: HikeTripMemberInput[], startAt: Date): Date {
  ...
  for (const m of ordered) {
    ...
    const end =
      isSet(m.duration) && m.duration > 0 ? new Date(start.getTime() + m.duration * 1000) : start;
