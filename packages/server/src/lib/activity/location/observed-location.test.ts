...
vi.mock('@sharpit/server/lib/geocoding/nominatim', () => ({
  reverseGeocode: vi.fn(),
}));

vi.mock('@sharpit/server/lib/environment/athlete-location', () => ({
...
describe('backfillActivityObservedLocation', () => {
  ...
  it('overrides a stale travel stamp when GPS stream (map) is present', async () => {
    const { resolveAthleteGeoLocation } =
      await import('@sharpit/server/lib/environment/athlete-location');
    const { reverseGeocode } = await import('@sharpit/server/lib/geocoding/nominatim');
    ...
    const { backfillActivityObservedLocation } =
      await import('@sharpit/server/lib/activity/location/observed-location');
