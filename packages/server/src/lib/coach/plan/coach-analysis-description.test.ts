...
} from '@sharpit/server/lib/coach/plan/coach-analysis';
...
vi.mock('@sharpit/server/lib/integrations/strava/strava', () => ({
...
describe('resolveAthleteDescription', () => {
  it('skips Strava when local notes are substantial', async () => {
    const { fetchActivityDetail } = await import('@sharpit/server/lib/integrations/strava/strava');
