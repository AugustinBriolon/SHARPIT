import { beforeEach, describe, expect, it, vi } from 'vitest';

vi.mock('@sharpit/db/client', () => ({
  prisma: {
    myFitnessPalAccount: {
      findUnique: vi.fn(),
      update: vi.fn(),
      upsert: vi.fn(),
      deleteMany: vi.fn(),
    },
    dailyNutrition: {
      upsert: vi.fn(),
    },
  },
}));

vi.mock('@sharpit/server/lib/engines/observation-engine', () => ({
  observationEngine: { ingest: vi.fn(), ingestBatch: vi.fn() },
}));

vi.mock('@sharpit/server/lib/integrations/myfitnesspal/myfitnesspal', () => ({
  MfpSessionExpiredError: class MfpSessionExpiredError extends Error {},
  fetchDiaryDay: vi.fn(),
  fetchDisplayName: vi.fn(),
  fetchNutrientGoals: vi.fn(),
  refreshMfpSession: vi.fn(),
}));

describe('syncMfpNutrition credential resilience', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.SECRET_ENCRYPTION_KEY = 'mfp-sync-test-key';
  });

  it('returns quietly when the account is already revoked (empty session)', async () => {
    const { prisma } = await import('@sharpit/db/client');
    const { syncMfpNutrition } =
      await import('@sharpit/server/lib/integrations/myfitnesspal/myfitnesspal-sync');

    vi.mocked(prisma.myFitnessPalAccount.findUnique).mockResolvedValue({
      athleteId: 'ath-1',
      sessionTokenEnc: '',
      displayName: 'Athlete',
    } as never);

    await expect(syncMfpNutrition('ath-1')).resolves.toEqual({ synced: 0, errors: 0 });
    expect(prisma.myFitnessPalAccount.update).not.toHaveBeenCalled();
  });

  it('does not revoke when ciphertext fails authenticity (wrong key)', async () => {
    const { prisma } = await import('@sharpit/db/client');
    const { syncMfpNutrition } =
      await import('@sharpit/server/lib/integrations/myfitnesspal/myfitnesspal-sync');
    const { encryptSecret, isSecretAuthenticityFailure } =
      await import('@sharpit/server/lib/secret-box');

    process.env.SECRET_ENCRYPTION_KEY = 'key-that-wrote-the-blob';
    const blob = encryptSecret('cookie');
    process.env.SECRET_ENCRYPTION_KEY = 'different-runtime-key';

    vi.mocked(prisma.myFitnessPalAccount.findUnique).mockResolvedValue({
      athleteId: 'ath-1',
      sessionTokenEnc: blob,
      displayName: 'Athlete',
    } as never);

    await expect(syncMfpNutrition('ath-1')).rejects.toSatisfy(isSecretAuthenticityFailure);
    expect(prisma.myFitnessPalAccount.update).not.toHaveBeenCalled();
  });
});
