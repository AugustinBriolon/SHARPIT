...
vi.mock('@sharpit/server/lib/integrations/garmin/garmin-mobile-auth', () => ({
...
vi.mock('@sharpit/server/lib/integrations/garmin/garmin-widget-auth', () => ({
...
vi.mock('@sharpit/server/lib/integrations/garmin/garmin-di-oauth', async () => {
  const actual = await vi.importActual<
    typeof import('@sharpit/server/lib/integrations/garmin/garmin-di-oauth')
  >('@sharpit/server/lib/integrations/garmin/garmin-di-oauth');
  return {
    ...actual,
...
describe('loginWithCredentials path order', () => {
  ...
  it('uses mobile SSO first and never calls widget when mobile succeeds', async () => {
    ...
    const { loginWithCredentials, isDiGarminTokens, diClientIdFromGarminTokens } =
      await import('@sharpit/server/lib/integrations/garmin/garmin');
  ...
  it('falls back to widget only when mobile is rate_limited', async () => {
    const { GarminMobileAuthError } =
      await import('@sharpit/server/lib/integrations/garmin/garmin-mobile-auth');
    ...
    const { loginWithCredentials } = await import('@sharpit/server/lib/integrations/garmin/garmin');
    const result = await loginWithCredentials('user@example.com', 'pw');
