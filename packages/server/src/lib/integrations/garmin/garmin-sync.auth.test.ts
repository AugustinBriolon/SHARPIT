...
import type { GarminTokens } from '@sharpit/server/lib/integrations/garmin/garmin';
...
vi.mock('@sharpit/server/lib/integrations/garmin/garmin', async () => {
  const actual = await vi.importActual<
    typeof import('@sharpit/server/lib/integrations/garmin/garmin')
  >('@sharpit/server/lib/integrations/garmin/garmin');
  return {
    ...actual,
...
describe('buildFreshGarminClient auth hardening', () => {
  ...
  it('revokes credentials when DI refresh fails (needs reconnect), without SSO', async () => {
    ...
    const { ProviderAuthError } =
      await import('@sharpit/server/lib/integrations/shared/connection-status');
