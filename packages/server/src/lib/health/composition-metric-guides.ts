...
import {
  ...
  afibToScaleValue,
} from '@sharpit/server/lib/integrations/withings/withings-ecg-display';
...
export const COMPOSITION_METRIC_GUIDES: Record<CompositionMetricId, MetricGuide> = {
  bmi: {
    ...
    interpret(value, ctx) {
      ...
      if (isSet(ctx.heightM) && isSet(ctx.weightKg)) {
  ...
  vascularAgeYears: {
    ...
    interpret(value, ctx) {
      ...
      const tone = isSet(ageTone) ? maxCorpsTone(zone.tone, ageTone) : zone.tone;
      return {
        zoneLabel: zone.label,
        tone,
