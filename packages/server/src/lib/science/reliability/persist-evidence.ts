...
import { runAnalysisRecalcWithRollback } from '@sharpit/server/lib/science/reliability/analysis-evidence';
import {
...
export async function persistEvidenceFromSnapshot(snapshot: AthleteSnapshot): Promise<void> {
  const { athleteHasHealthDataConsent } = await import('@sharpit/server/lib/privacy/consent-store');
