...
import { getEnvironmentalStressor } from '../../../environment';
...
import { ENVIRONMENTAL_NEUTRAL_ZONE } from '../../../environment/calibration';
import {
...
describe('environment scientific validation — boundary isolation', () => {
  const DOWNSTREAM_PATHS = [
    'src/inference/recovery/model.ts',
    'src/inference/fatigue/model.ts',
    'src/inference/adaptation/model.ts',
    'src/inference/reasoning/model.ts',
  ];
  ...
  it('inference layer does not modify environment-v1.1 public exports', async () => {
    const envIndex = readFileSync(resolve(process.cwd(), 'src/environment/index.ts'), 'utf8');
