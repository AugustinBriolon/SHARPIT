...
import type { FeatureEngine } from '../../../features/engine';
import type { DayFeatures, LoadFeatureSet, RecoveryFeatureSet } from '../../../features/types';
...
class InMemoryDigitalTwinRepository implements DigitalTwinRepository {
  ...
  async updatePhysicalHealth(
    athleteId: string,
    physicalHealthState: import('../../physical-health/types').PhysicalHealthState,
  ): Promise<DigitalTwin> {
    const existing = await this.findOrCreate(athleteId);
    const updated: DigitalTwin = {
      ...existing,
  ...
  async getPreviousPhysicalHealthState(
    athleteId: string,
  ): Promise<import('../../physical-health/types').PhysicalHealthState | null> {
    return this.store.get(athleteId)?.state.physicalHealth ?? null;
  }

  async updateEnvironmentalState(
    athleteId: string,
    state: import('../../../digital-twin/types').EnvironmentalTwinState,
  ): Promise<DigitalTwin> {
    const existing = await this.findOrCreate(athleteId);
    const updated: DigitalTwin = {
      ...existing,
  ...
  async getEnvironmentalState(
    athleteId: string,
  ): Promise<import('../../../digital-twin/types').EnvironmentalTwinState | null> {
    return this.store.get(athleteId)?.state.environmental ?? null;
  }

  async getEnvironmentalImpact(
    athleteId: string,
  ): Promise<import('../../../digital-twin/types').EnvironmentalTwinState['impact'] | null> {
