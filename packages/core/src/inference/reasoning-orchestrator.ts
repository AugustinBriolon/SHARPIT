...
export class ReasoningInferenceOrchestrator {
  ...
  async getLatest(
    ...
    const rawDecision = stateUpdate.decisionState as
      Omit<import('../decision').DecisionState, 'computedAt'> | undefined;

    const output: ReasoningModelOutput = {
      signals: record.signals as import('./reasoning/types').ReasoningSignals,
      reasoningState: {
        ...(stateUpdate as Omit<import('../digital-twin/types').ReasoningState, 'computedAt'>),
        computedAt: new Date((stateUpdate.computedAt as string) ?? record.computedAt),
      } as import('../digital-twin/types').ReasoningState,
      decisionState: rawDecision
        ? ({
          ...
          } as import('../decision').DecisionState)
        : ({
            ...(stateUpdate as Omit<import('../digital-twin/types').ReasoningState, 'computedAt'>),
            primaryDecision: {
              verdict:
                (stateUpdate.overallVerdict as import('../digital-twin/types').OverallVerdict) ??
                'INSUFFICIENT_DATA',
